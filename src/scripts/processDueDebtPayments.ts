// src/scripts/processDueDebtPayments.ts
// CLI runner for Phase 10 debt payment-plan processor.
// Dry-run by default. Use --write to create due debt payments.
// Examples:
//   npm run debts:process-due-payments
//   npm run debts:process-due-payments -- --workspaceId=... --asOfDate=2026-07-31
//   npm run debts:process-due-payments:write -- --workspaceId=...
...
import { processDueDebtPaymentsService } from "@/src/debts/services/debtPaymentPlanProcessor.service";
import type { ProcessDueDebtPaymentsResult } from "@/src/debts/types/debts.types";
...
function printWorkspaceResult(workspaceName: string, result: ProcessDueDebtPaymentsResult): void {
    ...
    for (const item of result.items) {
        console.log(
            `- ${item.action}: ${item.debtName} | scheduled ${item.scheduledPaymentDate.toISOString().slice(0, 10)} | amount ${item.amount.toFixed(2)} | principal ${item.principalAmount.toFixed(2)} | fee ${item.feeAmount.toFixed(2)}${item.nextDueDate ? ` | next ${item.nextDueDate.toISOString().slice(0, 10)}` : ""}${item.transactionId ? ` | transaction ${item.transactionId.toString()}` : ""}${item.paymentId ? ` | payment ${item.paymentId.toString()}` : ""}${item.reason ? ` | ${item.reason}` : ""}`
        );
    }
}
...
async function main(): Promise<void> {
    ...
    console.log("Due debt payments processor");
    ...
    try {
        ...
        if (options.dryRun) {
            console.log("\nDry-run only. Run with --write to generate debt payments.");
        } else {
            console.log("\nDue debt payment processing completed.");
        }
    } finally {
        await disconnectDb();
    }
...
main().catch((error: unknown) => {
    const message = error instanceof Error ? error.message : "Unknown due debt payment error.";

    console.error("\nDue debt payment processing failed:");
